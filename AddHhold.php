...
<p><font face = "cambria" size = 4 color = "grey"> New Address: </font></p>
	<div class="form-group">				
		   ...
		   <div class="col-sm-4">
             ...
             <input id="MName" class="form-control input-group-lg reg_name" type="text" name= "Street" title="Enter middle name" placeholder="Street Name"required <?php if(isset($_POST["Street"])) echo "value = ".$_POST["Street"]; ?> >
			 
           </div>
           <div class="col-sm-3">
		
             <input id="LName" class="form-control input-group-lg reg_name" type="text" name= "Purok" title="Enter last name" placeholder="Purok Number" required <?php if(isset($_POST["Purok"])) echo "value = ".$_POST["Purok"]; ?> >
           </div>
...
mysqli_query($con,"INSERT INTO `tblhousehold`( `Street`, `Purok`, `BuildingNo`, `HouseholdLname`,`Residence`,`OldAddress`,`Status`) VALUES ('$Street','$Purok','$Building','$_Lname','$Residence','$Old','Enabled')");
$query = mysqli_query($con,"Select Householdno from tblhousehold order by Householdno desc");
	...
	VALUES ('$_Fname','$_Mname','$_Lname','$Ename','$Gender','$Birthdate','$Contact','$Occupation','$SSS','$TIN','$no','$Civil','Head','Alive')");
	$_SESSION['Hno'] = $no;
