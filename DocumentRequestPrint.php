	...
	<form method="POST">
		...
		<?php
			...
			}else if(isset($_POST['ftrPaid']))
			{
				...
				$docReqPaymentSQL = "SELECT dr.strDocRequestID, CONCAT(h.strLastName, ', ', h.strFirstName, ' ', h.strMiddleName, ' ', h.strNameExtension) as 'Full Name', d.strDocName from tbldocumentrequest dr INNER JOIN tbldocument d on dr.strDRdocCode = d.intDocCode INNER JOIN tblhousemember h ON h.intMemberNo = dr.strDRapplicantID AND dr.strDocReqStat LIKE 'Approved'
			UNION  
...
UNION 
			...
			WHERE d.intDocCode = 3 AND bs.strClearanceStat = 'Approved'
            UNION
			SELECT bs.strBusStatID, CONCAT(a.`strLastName`, ', ', a.`strFirstName`, ' ', a.`strMiddleName`, ' ', a.`strNameExtension`) AS 'Name', d.strDocName AS Document FROM tblhousemember a INNER JOIN tblbusinessstat bs ON bs.strBusOwnerID = a.intMemberNo INNER JOIN tblpaymentdetail p ON bs.strBusStatID = p.strRequestID INNER JOIN tblbusiness b ON bs.strBusinessID = b.strBusinessID INNER JOIN tbldocument d ON d.intDocCode = bs.intBusinessDoc
			WHERE d.intDocCode = 7 AND bs.strClearanceStat = 'Approved'
			UNION
			...
			/*
			SELECT dr.strDocRequestID, CONCAT(h.strLastName, ', ', h.strFirstName, ' ', h.strMiddleName, ' ', h.strNameExtension) as 'Full Name', d.strDocName from tbldocumentrequest dr INNER JOIN tbldocument d on dr.strDRdocCode = d.intDocCode INNER JOIN tblhousemember h ON h.intMemberNo = dr.strDRapplicantID AND dr.strDocReqStat LIKE 'Paid' OR dr.strDocReqStat LIKE 'For approval'
			UNION  
SELECT vc.strVCplateNo, CONCAT(a.`strLastName`, ', ', a.`strFirstName`, ' ', a.`strMiddleName`, ' ', a.`strNameExtension`) AS 'Full Name', d.strDocName AS Document 
			FROM tblhousemember a INNER JOIN tblvehicleclearance vc ON a.intMemberNo = vc.strResidentID INNER JOIN tbltru tru ON vc.strVCplateNo = tru.strTRUplateNo INNER JOIN tblpaymentdetail p ON p.strRequestID = tru.strTRUplateNo INNER JOIN tblvehicle v ON v.strVplateNo = tru.strTRUplateNo, tbldocument d
...
UNION 
			...
			WHERE d.intDocCode = 3 AND bs.strClearanceStat = 'Paid' AND p.intRequestORNo = 0
			*/
		...
		<div class="panel panel-default"><!-- Default panel contents -->	
			...
			while($row = mysqli_fetch_array($query)){?>
				...
				<td><button class='btn btn-success btn-xs' data-toggle='modal' data-target='#myModal' value = "<?php echo $row[0]?>" name= 'btnPrint'> Print </button></td>
				<?php  echo"</tr>";
